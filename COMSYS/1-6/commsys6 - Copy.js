...
const favPresent = () =>{
if (activeUser.uFav['cIDX'].length>0
	||
    activeUser.uFav['rIDX'].length>0) {return true;} else {return false;}
};
const heartSwitch = () =>{
...
if (evTarget===null) {commntsAmount.innerHTML=`Комментарии <span class='commentsQuantity'>(${cBase.length})</span>`;
		      return;}
...
const prefixFv = `<img class='cmntFavFild' src='img/heart`, postfix1 = `.gif'>В избранно`,
      ...
      toFav = `${prefixFv}${inActFavPic}${postfix1}е`,  termDiv = '</div>', startDiv = '<div class=',
...
let replyToName = '* unknown *',
	    ...
	    reply=false, favItemOrder = 1, headerF0El, headerF0Property, headerF1Style; 
  ...
  if (strToNum(getProp(centeredWrap,'width'))===376) {bFC.push('+');	bFC.push(base[IDX]['rate']); 	bFC.push('-');}
						else 
						     {bFC.push('-');	bFC.push(base[IDX]['rate']); 	bFC.push('+');}
...
if ((bFC==='+' || bFC==='-') && !rateCtrlWrap) {rateCtrlWrap = putDiv("rateCtrlWrap",contentBottom);}
if (rateCtrlWrap) {
...
if (bFC==='+' || bFC==='-') {
	cBFs.borderRadius = '50%'; cBFs.backgroundColor = 'rgb(230,230,230)';
      if (bFC==='+') {cBFs.color=plusColor;} else {cBFs.color=minusColor;}
 ...
 cBFs.width = iHeight+px;
   contentBottomF.addEventListener('click',(ev) =>{	
	...
	if (rateCtrlWrap) {	
			...
			cBFs.color=plusColor; cBFs.fontWeight='600';}
...
/* show symbol counter or warning message at inputFormHeader */		inputFormMain.appendChild(txtInput);
...
const activeUserName = () =>{
	...
	textLen = text.length+ver.length+1,  nameFieldStyle = doc.querySelector('.nameField').style;	
...
/* user switch buttons */
uBase.forEach((i)=>{let btntxtInpStyle;
		    lastButton=putBtn('eventBtn1',usrSlct,i['name'],(ev)=>{
				newUserLogIn(i['name']);
				btntxtInpStyle = ev.target.style;	btntxtInpStyle.fontWeight='700';	btntxtInpStyle.boxShadow='0px 0px 3px 2px yellow';	
								btntxtInpStyle.borderColor='lime';	btntxtInpStyle.background='greenyellow';								
		    });		btntxtInpStyle = lastButton.style;		
								btntxtInpStyle.marginRight='10px';	btntxtInpStyle.background='limegreen';
...
const newUserLogIn = (userName) => {
			...
			eventBtns1.forEach((i,n)=>{
				    ...
				    nameField.innerHTML = activeUserName()+ver;
};
